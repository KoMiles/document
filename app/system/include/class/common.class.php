...
class common {
	...
	protected function load_config_global() {
		...
		$_M['config']['met_webkeys'] = file_get_contents(PATH_WEB.'/config/config_safe.php');
	...
	protected function load_word($lang, $site) {
		global $_M;
		$query = "SELECT * FROM {$_M['table']['language']} WHERE lang='{$lang}' AND site='{$site}' AND array!='0'";
		$result = DB::query($query);
	...
	protected function template($path){
		...
		if($postion == 'tem'){
			if (M_MOUDLE == 'admin') {
				return PATH_SYS."admin/template/met/{$file}.php";
			} else {
