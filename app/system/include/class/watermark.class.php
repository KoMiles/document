 ...
 * @param string $water_text_font		水印文本字体(字体文件要放在后台管理目录include/fonts/)
...
class watermark{
	...
	public function create($water_scr_image) {
		...
		if($this->is_watermark != 1)return path_relative($water_scr_image);
		...
		if ($this->water_save_type == 3) {
			if (basename($this->water_savepath)) {
				$save_path_water = dirname($this->water_savepath);
			} else {
				$save_path_water = dirname($this->water_savepath);
			}
		}
	 ...
	 * @return array 返回错误信息
	 */
	protected public function error($error){
		$back['error'] = 1;
	 ...
	 */
	protected public function sucess($path){
		$back['error']=0;
		$back['path']=$path;
		return $back;
	}
