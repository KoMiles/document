...
/**
  ...
  * @param  int    $type     生成字符串类型,0(默认):26字母大小写+数字,1:数字,2:26字母大写,3:26字母大写,4:26字母大小写
  * @return string           返回产生随机字符串
  */
function random($length = 1,$type = 0) {
	switch ($type) {
		case 1:
			$patten = '0123456789';
		break;
...
function url_standard($url){
	...
	$start=strripos($url,'index.');
	if($start !== false){
		$end = stripos($url,'?');
		$end = $end !== false ? $end-$start-6 : '';
		$suffix = $end ? substr($url,$start+6,$end) : substr($url,$start+6);
...
function httphead_info(){
	...
	$headinfo['Previous'] = $_SERVER['HTTP_REFERER'];//前一个页面地址
