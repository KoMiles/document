...
function metlabel_navnow($type=1,$label='',$indexnum,$listyy=0,$listmax=8,$msow=0){
	global $sidedwon2,$sidedwon3,$index_url,$nav_list,$nav_list2,$nav_list3,$class1,$class_list,$module_list1,$class_index,$classlistall,$lang,$db,$met_column;
	...
	foreach($nav_list2[$class] as $key=>$val){
		if($val[display]==1){
			unset($nav_list2[$class][$key]);
		}
		foreach($nav_list3[$val[id]] as $key1=>$val1){
			if($val1[display]==1){
			unset($nav_list3[$val[id]][$key1]);
		}
		}
	}
...
function metlabel_product($z,$w,$l,$n=0){
	global $product_list,$metblank,$met_img_style,$met_img_x,$met_img_y,$met_product_page,$class1,$class2,$class3,$search,$nav_list2,$nav_list3,$weburly,$id,$met_agents_img,$class_list,$module_listall,$db,$lang,$met_parameter,$met_plist,$prices,$action;
...
function metlabel_messagelist(){
	global $lang,$message_list,$lang_SubmitContent,$lang_Reply,$total_count,$from_record,$db,$met_config,$met_mlist;
	...
	$messagesNames1=$db->get_one("select * from $met_mlist where listid='$val[id]' and paraid='$messagesName1[value]' and lang='$lang'");
	$messagesName2=$db->get_one("select * from $met_config where name='met_message_fd_content' and lang='$lang'");
	$messagesNames2=$db->get_one("select * from $met_mlist where listid='$val[id]' and paraid='$messagesName2[value]' and lang='$lang'");
	$metinfo.="<dl class='list-none metlist'>\n";
...
function metlabel_message($fid,$mobile){
	...
	if($mobile){
     ...
     $metinfo =$fdjs;
     $metinfo.="<form enctype='multipart/form-data' method='POST' name='myform' onSubmit='return metmessagesubmit();' action='{$lujin}message.php?action=add&lang=".$lang."' target='_self'>\n";
     $metinfo.="<table class='feedback_table' >\n";
    foreach($fd_para as $key=>$val){
     $metinfo.="<tr>\n";
     $metinfo.="<td class='text'>".$val[name]."</td>\n";
...
function metlabel_messageold(){
	global $lang,$message_list,$lang_Submit,$lang_Reset,$lang_Publish,$lang_Reply,$fromurl,$m_user_ip,$id,$title,$lang_SubmitInfo;
	global $met_memberlogin_code,$lang_memberImgCode,$lang_memberTip1,$met_adminfile,$navurl,$settings_arr;
	global $db,$met_parameter,$met_admin_array,$met_member_use,$metinfo_member_type,$met_list,$met_class,$class_list,$met_product,$lang_Choice,$lang_Empty;
	$metinfo="<form method='POST' name='myform' onSubmit='return Checkmessage();' action='message.php?action=add' target='_self'>\n";
	$metinfo.="<table width='90%' cellpadding='2' cellspacing='1' bgcolor='#F2F2F2' align='center' class='message_table'>\n";
	$query = "SELECT * FROM $met_parameter where lang='$lang' and  module=7 and class1='$id' order by no_order";
	if($met_member_use){
		$query = "select * from $met_parameter where (access in(select id from $met_admin_array where user_webpower<='$metinfo_member_type') or access=0) and lang='$lang' and module=7 and class1='$id' order by no_order;";
	}
	$result = $db->query($query);
	while($list= $db->fetch_array($result)){
		if($list[type]==2 or $list[type]==4 or $list[type]==6){
